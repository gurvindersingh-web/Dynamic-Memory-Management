  ...
  useGSAP(
    () => {
      ...
      const cleanupToStill = () => {
        // Strip extra cloned chars from each wrapper, keeping only the
        // original char visible. We intentionally keep the wrapper structure
        // in the DOM — this is critical for gradient text-fill-color styles
        // (background-clip: text + transparent fill) which require chars
        // to stay within the element's DOM tree to render the gradient.
        wrappersRef.current.forEach(w => {
          const strip = w.firstElementChild;
          if (!strip) return;
      ...
      const play = () => {
        ...
        const tl = gsap.timeline({
          ...
          onComplete: () => {
            ...
            if (!loop) {
              cleanupToStill();
              if (colorTo && ref.current) gsap.set(ref.current, { color: colorTo });
              onShuffleComplete?.();
              armHover();
            }
          }
